...
### 3.1 浏览器内置Web Speech API实现

本项目使用浏览器内置的Web Speech API实现语音识别功能，无需第三方API依赖，直接在前端实现语音到文本的转换。

#### 3.1.1 实现说明

1. **浏览器兼容性**
   - 支持Chrome、Edge、Safari 14+等现代浏览器
   - 自动检测浏览器支持情况，不支持时显示提示信息

2. **核心功能实现**

语音识别服务通过speechRecognitionService.js模块提供，主要包含以下功能：
...
const recognizeSpeech = async () => {
  ...
  recognition.lang = 'zh-CN'; // 配置为中文识别
...
1. **HTTPS要求**
   - 浏览器要求在HTTPS环境下才能使用麦克风权限
   - 确保生产环境使用HTTPS协议

2. **用户隐私**
   - 语音数据仅在用户浏览器本地处理，不会上传到第三方服务器
   - 确保用户了解语音识别功能的隐私政策
